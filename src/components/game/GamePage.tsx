          ...
          <div className="hidden sm:block w-[500px] p-0 2xl:hidden">
            <div className="flex flex-col justify-center items-center rounded-xl">
              ...
              <div className="flex flex-col">
                <h1 className="font-bold text-white text-[20px]">Publisher: {publishers.map((p) => p.name).join(", ")}</h1>
                <Separator  className="mt-[10px]" orientation="horizontal"/>
                <Link className="mt-[10px]" href={website}><h1 className=" text-white text-[20px] underline">See more about the game on their website: {website}</h1></Link>
                <Separator  className="mt-[10px]" orientation="horizontal"/>
                <h1 className="text-white mt-[10px] text-[20px]">Tags: {tags.map(t => t.name).join(', ')}</h1>
                <Separator  className="mt-[10px]" orientation="horizontal"/>
                <h1 className="text-white mt-[10px] text-[20px]">Available on {singleArray.map((s) => s.name).join(", ")}</h1>
                <Separator  className="mt-[10px]" orientation="horizontal"/>
                <h1 className="text-white mt-[10px] text-[20px]">Platforms on {singlePlatform.map((p) => p.name).join(", ")}</h1>
              </div>
            </div>
          </div>
        </div>
